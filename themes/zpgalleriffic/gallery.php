<?php include ("header.php"); ?>
	...
	<?php if (is_numeric($zpgal_minigalheight)) {$minigalimageheight=$zpgal_minigalheight;$minigalheight=$minigalimageheight+3;} else {$minigalimageheight=250;$minigalheight=$minigalimageheight+3;} ?>
	<?php if (is_numeric($zpgal_minigalcount)) {$minigalcount=$zpgal_minigalcount;} else {$minigalcount=12;} ?>
	<style>
	...
	<div class="wrapper" id="minigal">
		<div class="centered">	
			<div id="minigal-title">
				<?php switch ($zpgal_minigaloption) {
					case "random":
				...
				<div class="navigation-container">
					<div id="thumbshome" class="navigation">
						...
						<ul class="thumbs noscript">
							<?php if (($zpgal_minigaloption) != 'random') { ?>
							...
							<li>
								...
								<div class="caption">
									<div class="download">
										<a href="<?php echo html_encode($image->getImageLink()); ?>" title="<?php echo html_encode($image->getTitle()); ?>"></a>
									</div>
								</div>
							</li>
							<?php } ?>
							<?php } else { ?>
							<?php for ($i=1; $i<=$minigalcount; $i++) { 
							...
							<li>
								...
								<div class="caption">
									<div class="download">
										<a href="<?php echo html_encode($randomImage->getImageLink()); ?>" title="<?php echo html_encode($randomImage->getTitle()); ?>"></a>
									</div>
								</div>
							</li>
							<?php } ?>
	...
	<div class="wrapper">
		<div class="centered">	
			...
			<div id="sidebar">
				<?php if (is_numeric($zpgal_zp_latestnews_trunc)) { $latesttrunc = $zpgal_zp_latestnews_trunc; } else { $latesttrunc = 400; } ?>
				<?php printLatestNews( $zpgal_zp_latestnews,'none','',true,true,$latesttrunc,true ); ?>
			</div>
			...
			<div id="album-wrap" <?php if ( (function_exists('printLatestNews')) && ($zpgal_zp_latestnews > 0) ) { ?>class="withsidebar"<?php } ?>>
				<ul>
					...
					<li<?php echo $lastcol; ?>>	
						<?php if (strlen(getAlbumDesc()) > 0) { ?><a class="album-thumb" href="<?php echo htmlspecialchars(getAlbumLinkURL());?>" title="<?php echo gettext('View album:'); ?> <?php echo getBareAlbumTitle();?>"><?php printCustomAlbumThumbImage(getBareAlbumTitle(),NULL,238,100,238,100); ?></a>
						<?php } else { ?>
						...
						<?php } ?>
						<h4><a href="<?php echo htmlspecialchars(getAlbumLinkURL());?>" title="<?php echo gettext('View album:').getBareAlbumTitle(); ?>"><?php echo shortenContent(getBareAlbumTitle(),30,'...'); ?></a></h4>
