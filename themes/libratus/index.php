...
<?php if ($_zp_page == 1) { ?>
	...
	<ul id="cbp-bislideshow" class="cbp-bislideshow">
		...
		foreach ($images as $image) {
			...
			$html = '<img src="' . html_encode(pathurlencode($image->getCustomImage(1200,
																					NULL,
																					NULL,
																					NULL,
																					NULL,
																					NULL,
																					NULL,
																					TRUE))) . '" title="' . $url . '" albumTitle="' . html_encode($album->getTitle()) . '"  alt="' . html_encode($image->getTitle()) . '" />';
...
<?php } ?>
	...
	<div id="home-bar">
		...
		<div class="bar">
			<div class="inner">
				<?php echo $quickmenu; ?>
				<div class="pad" id="breadcrumb"><a href="<?php echo getGalleryIndexURL(); ?>"><i
							class="fa fa-home"></i>&nbsp;<?php printGalleryTitle(); ?></a></div>
			</div>
		</div>
	</div>
	...
	<div id="main" class="wrap clearfix">
		<div class="inner">
			<div class="gallery pad<?php if (getOption('libratus_index_fullwidth')) {
				echo ' fullwidth';
			} ?>">
					...
					<?php while (next_album()): ?>
						<div>
							...
							<div class="caption clearfix">
								<?php if (getOption('libratus_date_albums')) { ?>
									<div class="album-date"><?php printAlbumDate(); ?></div><?php } ?>
				...
				<?php if (hasNextPage() || hasPrevPage()) { ?>
					<div class="pad">
					<hr/><?php printPageListWithNav('« ' . gettext('prev'),
													gettext('next') . ' »',
													FALSE,
													TRUE,
													'pagination'); ?></div><?php } ?>
			...
			<?php if (!getOption('libratus_index_fullwidth')) { ?>
				<div class="gallery-sidebar pad">
					<?php printSearchForm('',
										  'search',
										  $_zp_themeroot . '/images/magnifying_glass_16x16.png',
										  gettext('Search gallery'),
										  $_zp_themeroot . '/images/list_12x11.png'); ?>
					<hr/>
