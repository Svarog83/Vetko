...
if (is_null(getOption('zpskel_thumbsize'))) { $optionsnotsaved = true; } else { $optionsnotsaved = false; } // test this option to determine if theme options have been saved at least once

// warning about plugins
if (!$zpskel_disablewarning){
$plugincount = 0;
if (getOption('zp_plugin_colorbox')) { $warning_listitem .= '<li>Disable Plugin: <strong><em>Colorbox</em></strong> [ This theme uses and configures it\'s own javascript popup plugin for image previews ]</li>'; $plugincount++; }
...
if (getOption('zp_plugin_paged_thumbs_nav')) { $warning_listitem .= '<li>Disable Plugin: <strong><em>Paged Thumb Nav</em></strong> [ Sorry no theme support for this plugin. ]</li>'; $plugincount++; }
if (getOption('zp_plugin_menu_manager')) { $warning_listitem .= '<li>Disable Plugin: <strong><em>Menu Manager</em></strong> [ Sorry no theme support for this plugin. ]</li>'; $plugincount++; }
if (getOption('zp_plugin_slideshow')) { $warning_listitem .= '<li>Disable Plugin: <strong><em>Slideshow</em></strong> [ This theme uses and configures it\'s own javascript popup plugin for slideshows ]</li>'; $plugincount++; }
...
$zpskel_social = getOption('zpskel_social'); if (is_null($zpskel_social)) $zpskel_social = true;
$zpskel_debuguser = getOption('zpskel_debuguser'); if (is_null($zpskel_debuguser)) $zpskel_debuguser = false;
$zpskel_usenews = getOption('zpskel_usenews'); if (is_null($zpskel_usenews)) $zpskel_usenews = true;
$zpskel_pptarget = getOption('zpskel_pptarget'); if (is_null($zpskel_pptarget)) $zpskel_pptarget = 'sized';
$zpskel_strip = getOption('zpskel_strip'); if (is_null($zpskel_strip)) $zpskel_strip = 'latest';
$zpskel_download = getOption('zpskel_download'); if (is_null($zpskel_download)) $zpskel_download = true;
$zpskel_thumbsize = getOption('zpskel_thumbsize'); if (is_null($zpskel_thumbsize)) $zpskel_thumbsize = 'large';
$zpskel_archive = getOption('zpskel_archive'); if (is_null($zpskel_archive)) $zpskel_archive = true;
$zenpage_homepage = getOption('zenpage_homepage'); if (is_null($zenpage_homepage)) $zenpage_homepage = 'none';
...
$browser = new Browser();
If ($browser->isMobile()) { $zpskel_ismobile = true; } else { $zpskel_ismobile = false; }
