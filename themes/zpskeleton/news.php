	...
	<div class="wrapper contrast top">
		<div class="container">	
			<div class="sixteen columns">
				...
				<h5><?php printNewsIndexURL(gettext('News')); echo ' &raquo; '; ?>(<?php printNewsCategories(', ','','taglist'); ?>)</h5>
				...
				<h5><?php printNewsIndexURL(gettext('News')); echo ' &raquo; '; ?></h5>
			...
			<div class="ten columns">
					...
					<div class="newsarticle clearfix">
						...
						<div class="news-meta">
							<ul class="taglist">
								<li class="meta-date"><?php printNewsDate();?></li>
								<li class="meta-comments"><?php echo getCommentCount().' '.gettext('Comment(s)'); ?></li>
								<?php if(is_GalleryNewsType()) {
									if(!is_NewsType("album")) {
										echo "<li class=\"meta-contents\">".gettext("Album:")." <a href='".getNewsAlbumURL()."' title='".getBareNewsAlbumTitle()."'>".getNewsAlbumTitle()."</a></li>";
									}
								} else {
									echo '<li class="meta-cats">'; printNewsCategories(', ','','taglist'); echo '</li>';
								} ?>
								<?php $singletag = getTags(); $tagstring = implode(', ', $singletag); ?>
								<?php if (strlen($tagstring) > 0) { ?><li class="meta-tags"><?php printTags('links','','taglist', ', '); ?></li><?php } ?>
							</ul>
				...
				<?php endwhile; ?>
					...
					<div class="pagination">
						<?php printNewsPageListWithNav('&raquo;','&laquo;',true,'pagination',true,5); ?>
					</div>
					<?php } ?>
				<?php } ?>
			</div>
		...
		<div class="container">
			<div class="sixteen columns">
				...
				<ul class="taglist rss">
					<?php if (function_exists('printCommentForm')) { ?><li><?php printZenpageRSSLink('Comments-news','','',gettext('Comments of this article'),'',false); ?></li><?php } ?>
				</ul>
			</div>
		</div>
	</div>
	...
	<div class="wrapper">
		<div class="container">
			<div class="sixteen columns">
				...
				<?php } ?>
				<?php if (function_exists('printCommentForm')) { printCommentForm(); echo '<hr />'; } ?>
			</div>
		</div>
	</div>
	<?php } ?>	
