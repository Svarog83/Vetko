...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		<?php define('zpmasFULLWEBPATH', PROTOCOL."://" . $_SERVER['HTTP_HOST'] );
		$sociallink = urlencode(FULLWEBPATH);
		$socialtitle = urlencode(getBareGalleryTitle());
		...
		switch ($_zp_gallery_page) {
			...
			case 'album.php':
				$galleryactive = true;
				$zpmas_metatitle = getBareAlbumTitle().' | ';
				$zpmas_metadesc = truncate_string(getBareAlbumDesc(),150,'...');
				$sociallink = urlencode(zpmasFULLWEBPATH.getAlbumLinkURL()); 
				$socialtitle = urlencode(getBareAlbumTitle()." - ".strip_tags(truncate_string(getBareAlbumDesc(),75,'...')));
			...
			case 'image.php':
				$galleryactive = true;
				$zpmas_metatitle = getBareImageTitle().' | ';
				$zpmas_metadesc = truncate_string(getBareImageDesc(),150,'...');
				$sociallink = urlencode(zpmasFULLWEBPATH.getImageLinkURL()); 
				$socialtitle = urlencode(getBareImageTitle().": ".strip_tags(truncate_string(getBareImageDesc(),75,'...')));
				printRSSHeaderLink('Comments-image',getBareImageTitle().' - '.gettext('Latest Comments'), $lang='')."\n";
				break;
			case 'archive.php':
				$zpmas_metatitle = gettext("Archive View").' | ';
				break;
			case 'search.php':
				$galleryactive = true;
				$zpmas_metatitle = gettext('Search')." | ".getSearchWords().' | ';
				break;
			case 'pages.php':
				$zpmas_metatitle = getBarePageTitle().' | ';
				$zpmas_metadesc = strip_tags(truncate_string(getPageContent(),150,'...'));
				$sociallink = urlencode(zpmasFULLWEBPATH.getPageLinkURL(getPageTitleLink())); 
				$socialtitle = urlencode(getBarePageTitle()." - ".strip_tags(truncate_string(getPageContent(),75,'...')));
				break;
			case 'news.php':
				setOption('zenpage_combinews',0,false);
				if (is_NewsArticle()) {
				$zpmas_metatitle = gettext('News').' | '.getBareNewsTitle().' | ';
				$zpmas_metadesc = strip_tags(truncate_string(getNewsContent(),150,'...'));
				$sociallink = urlencode(zpmasFULLWEBPATH.getNewsURL(getNewsTitleLink())); 
				$socialtitle = urlencode(getBareNewsTitle()." - ".strip_tags(truncate_string(getNewsContent(),75,'...')));
				} else if ($_zp_current_category) {
				$zpmas_metatitle = gettext('News').' | '.$_zp_current_category->getTitle().' | ';
				$zpmas_metadesc = strip_tags(truncate_string(getNewsCategoryDesc(),150,'...'));
				} else if (getCurrentNewsArchive()) {
				$zpmas_metatitle = gettext('News').' | '.getCurrentNewsArchive().' | ';
				} else {
				$zpmas_metatitle = gettext('News').' | ';
				}
				break;
			case 'contact.php':
				$zpmas_metatitle = gettext('Contact').' | ';
				break;
			case 'register.php':
				$zpmas_metatitle = gettext('Register').' | ';
				break;
			case 'password.php':
				$zpmas_metatitle = gettext('Password Required').' | ';
				break;
			case '404.php':
				$zpmas_metatitle = gettext('404 Not Found...').' | ';
				break;
			default:
				$zpmas_metatitle = '';
				$zpmas_metadesc = truncate_string(getBareGalleryDesc(),150,'...');
				$sociallink = urlencode(zpmasFULLWEBPATH);
				$socialtitle = urlencode(getBareGalleryTitle());
				break;
		} 
		...
		if ($zenpage) {
			...
			printZenpageRSSHeaderLink('Comments-all','',gettext('Latest Comments'))."\n";
		} else {
		printRSSHeaderLink('Comments',gettext('Latest Comments'))."\n";
		} ?>
		<title><?php echo $zpmas_metatitle.getBareGalleryTitle(); ?></title>
		...
		<?php } ?>
		<link rel="stylesheet" href="<?php echo $_zp_themeroot; ?>/css/cbStyles/<?php echo $zpmas_cbstyle; ?>/colorbox.css" type="text/css" media="screen"/>
	...
	<body>
		...
		<div id="header">
			...
			<div id="header-inner">		
				<?php if ($zpmas_css == 'dark') { printSearchForm( '','searchform','',gettext('Search'),"$_zp_themeroot/images/media-eject-inv.png",null,null,"$_zp_themeroot/images/cancel-inv.png" ); } else { printSearchForm( '','searchform','',gettext('Search'),"$_zp_themeroot/images/media-eject.png",null,null,"$_zp_themeroot/images/cancel.png" ); } ?>
