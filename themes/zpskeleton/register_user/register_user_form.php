...
?>
	<form id="mailform" class="register_user remove-bottom" action="<?php echo sanitize($_SERVER['REQUEST_URI']); ?>" method="post" autocomplete="off">
		...
		<div>
			<label for="adminuser"><?php if (getOption('register_user_email_is_id')) echo gettext("Email:"); else echo gettext("User ID:"); ?></label>
		...
		<div>
			<?php $captchaCode = generateCaptcha($img); ?>
			<label for="code">
				<?php echo gettext("Enter CAPTCHA<sdivong>*</sdivong>:"); ?>
			</label>
			<img class="captcha-img" src=<?php echo "\"$img\"";?> alt="Code" />
			<input class="captcha-input" type="text" id="code" name="code" size="50" />
